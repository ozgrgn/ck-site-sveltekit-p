<script>
	...
	let values = [
		...
		{ key: 'workWithHeader', customValue: null },
		{ key: 'workWithSpot', customValue: null },
		{ key: 'workWithText', customValue: null },
		{ key: 'workWithImages', customValue: null },
	...
	const updateHomePage = async () => {
		homePage.workWithImages.value = homePage.workWithImages;
	...
	const getHomePage = async () => {
		...
		if (response['status']) {
			...
			if (!homePage.workWithImages.value) {
				homePage.workWithImages = [];
			} else {
				homePage.workWithImages = homePage.workWithImages.value;
			}
		} else {
			ToastService.error($TranslateApiMessage(response.message));
		}
	...
	let deleteImage = (index) => {
		homePage.workWithImages.splice(index, 1);
		homePage.workWithImages = homePage.workWithImages;
	};
</script>
					...
					<div class="flex flex-wrap my-4">
						...
						<div class="w-full lg:w-4/12 px-4 self-center">
							<div class="relative w-full mb-3">
								...
								<Input
									bind:value={homePage.workWithHeader.value}
									bind:isValid={homePage.workWithHeader.isValid}
									placeholder={'Başlık Giriniz'}
									required={false}
								/>
							</div>
						</div>
						<div class="w-full lg:w-8/12 px-4 self-center">
							<div class="relative w-full mb-3">
								<label class="block text-blueGray-600 text-xs font-bold mb-2" for="grid-name">
									Hastaneler Spot
								</label>
								<Input
									bind:value={homePage.workWithSpot.value}
									bind:isValid={homePage.workWithSpot.isValid}
									placeholder={'Spot Giriniz'}
									required={false}
								/>
							</div>
						</div>
						<div class="w-full lg:w-6/12 px-4">
							<div class="relative w-full mb-3">
								...
								<TextEditor
									placeholder={'Text'}
									bind:value={homePage.workWithText.value}
									bind:incomingValue={homePage.workWithText.value}
								/>
							</div>
						</div>
						<div class="w-full lg:w-6/12 px-4">
							<div class="relative w-full mb-3">
								<button
									on:click={() =>
										(homePage.workWithImages = [...homePage.workWithImages, { image: null }])}
									...
									type="button"
								>
									Hastane Resmi Ekle (600x400)
								</button>
								<div>
									{#if homePage.workWithImages[0]}
										{#each homePage.workWithImages as image, index}
											<div class="border mt-2 p-1 grid grid-cols-4">
												...
												<div class="col-span-2">
													<ImageArray bind:value={homePage.workWithImages[index].image} {index} />
												</div>
