<script>
	...
	const getHomePages = async () => {
		let getHomePagesResponse = await RestService.getHomePages(limit, skip);
		if (getHomePagesResponse['status']) {
			homePages = getHomePagesResponse['homePages'];
			totalDataCount = getHomePagesResponse['count'];
		} else {
			...
			<div class="rounded-t mb-0 px-4 py-3 border-0">
				<div class="flex flex-wrap items-center">
					<div class="relative w-full px-4 max-w-full flex-grow flex-1">
						<h3 class="font-semibold text-lg text-blueGray-700">Ana Sayfa</h3>
					</div>
				</div>
			</div>
								...
								<tr>
									...
									>
										<button
											...
											type="button"
											on:click={goto(`/panel/homePage/update/${homePage._id.toString()}`)}
										>
											Düzenle
										</button>
										<button
											on:click={() => deleteHomePageApprove(homePage._id.toString())}
											class="bg-white text-blue-600 hover:bg-[#6e6e85] hover:text-white border border-blue-600 rounded font-bold  text-xs px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none "
											type="button"
										>
											Sil
