<script>
	...
	const deleteBlogsPageApprove = (blogsPageId) => {
		modal.set(
			bind(Alert, {
				message: 'Bu işlemi onaylıyor musunuz ?',
				answer: (message) => {
					if (message) {
						deleteBlogsPage(blogsPageId);
					}
					modal.set(null);
				}
			})
	...
	const updateBlogsPage = async () => {
		let editedBlogsPage = {};
		values.map((v) => {
			editedBlogsPage[v.key] = blogsPage[v.key].value;
		});

		let response = await RestService.updateBlogsPage(blogsPage._id, editedBlogsPage);
		if (response['status']) {
	...
	const getBlogsPage = async () => {
		let response = await RestService.getBlogsPage(data.blogsPageId);

		if (response['status']) {
			values.map((v) => {
				if (v.customValue) {
					response['blogsPage'][v.key] = {
						value: response['blogsPage'][v.key][v.customValue]
					};
				} else {
					response['blogsPage'][v.key] = { value: response['blogsPage'][v.key] };
				}
			});
			blogsPage = {
				...response['blogsPage']
	...
	const deleteBlogsPage = async (blogsPageId) => {
		let response = await RestService.deleteBlogsPage(blogsPageId);
		if (response['status']) {
			ToastService.success('İşlem başarılı');
			navigate('/panel/blogsPage');
		} else {
			ToastService.success('İşlem başarılı');
		}
	};
	...
	<div class="w-full mb-12 px-2 lg:px-4 ">
		<div class="flex justify-between">
			...
			<button
				class="bg-white text-blue-600 hover:text-red-700 mb-2 border rounded font-bold text-xs px-4 py-2 rounded shadow hover:shadow-lg outline-none focus:outline-none mr-1 "
				type="button"
				on:click={() => deleteBlogsPageApprove(data.blogsPageId)}
			>
				<i class="fa fa-trash" />
				Sil
			</button>
		...
		>
			<div class="rounded-t mb-0 px-4 py-3 border-0">
				<div class="text-center flex justify-between">
					<h3 class="font-semibold text-lg text-blueGray-700">Blog Sayfasını güncelle</h3>
				</div>
			</div>
				...
				{#if blogsPage}
					<div class="flex flex-wrap my-4">
						...
						<div class="w-full lg:w-8/12 px-4">
							<div class="relative w-full mb-3">
								<label class="block text-blueGray-600 text-xs font-bold mb-2" for="grid-name">
									Meta Açıklaması Max 150 ({blogsPage?.metaDescription?.value?.length
