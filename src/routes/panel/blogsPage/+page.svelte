<script>
	...
	const getBlogsPages = async () => {
		let getBlogsPagesResponse = await RestService.getBlogsPages(limit, skip);
		if (getBlogsPagesResponse['status']) {
			blogsPages = getBlogsPagesResponse['blogsPages'];
			totalDataCount = getBlogsPagesResponse['count'];
		} else {
			...
			<div class="rounded-t mb-0 px-4 py-3 border-0">
				<div class="flex flex-wrap items-center">
					<div class="relative w-full px-4 max-w-full flex-grow flex-1">
						<h3 class="font-semibold text-lg text-blueGray-700">Blog Sayfaları</h3>
					</div>
				</div>
			</div>
									...
									<tr>
										...
										>
											<button
												...
												type="button"
												on:click={goto(`/panel/blogsPage/update/${blogsPage?._id.toString()}`)}
											>
												Düzenle
											</button>
											<button
												on:click={() => deleteBlogsPageApprove(blogsPage?._id.toString())}
												class="bg-white text-blue-600 hover:bg-[#6e6e85] hover:text-white border border-blue-600 rounded font-bold  text-xs px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none "
												type="button"
											>
												Sil
