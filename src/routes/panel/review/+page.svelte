<script>
	...
	const getReviews = async () => {
		let getReviewsResponse = await RestService.getReviews(lang, limit, skip);
		if (getReviewsResponse['status']) {
			console.log(getReviewsResponse, 'getReviewsResponse');
			reviews = getReviewsResponse['reviews'];
			totalDataCount = getReviewsResponse['count'];
		} else {
							...
							{#each reviews as review}
								<tr>
									...
									<td
										class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-center"
									>
										{review.reviewPlatform ? review.reviewPlatform : '-'}
									</td>
									<td
										class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-center"
									>
									...
									>
										<button
											...
											type="button"
											on:click={goto(`/panel/review/update/${review._id.toString()}`)}
										>
											Düzenle
										</button>
										<button
