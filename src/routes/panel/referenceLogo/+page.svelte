<script>
	...
	const getReferenceLogos = async () => {
		let getReferenceLogosResponse = await RestService.getReferenceLogos(limit, skip);
		if (getReferenceLogosResponse['status']) {
			referenceLogos = getReferenceLogosResponse['referenceLogos'];
			totalDataCount = getReferenceLogosResponse['count'];
		} else {
...
<div class="flex flex-wrap mt-4 h-screen relative">
	<div class="w-full mb-12 px-2 lg:px-4 ">
		<button
			...
			type="button"
			on:click={() => {
				goto('/panel/referenceLogo/create');
			}}
		>
			<i class="fa fa-plus" />
			Ekle
			...
			<div class="rounded-t mb-0 px-4 py-3 border-0">
				<div class="flex flex-wrap items-center">
					<div class="relative w-full px-4 max-w-full flex-grow flex-1">
						<h3 class="font-semibold text-lg text-blueGray-700">Referans Logosu</h3>
					</div>
				</div>
			</div>
								...
								>
									...
									>
										<button
											...
											type="button"
											on:click={goto(`/panel/referenceLogo/update/${referenceLogo._id.toString()}`)}
										>
											Düzenle
										</button>
										<button
