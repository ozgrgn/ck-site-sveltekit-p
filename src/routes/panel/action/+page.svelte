<script>
	...
	const getActions = async () => {
		let getActionsResponse = await RestService.getActions(limit, skip);
		if (getActionsResponse['status']) {
			console.log(getActionsResponse, 'getActionsResponse');
			actions = getActionsResponse['actions'];
			totalDataCount = getActionsResponse['count'];
		} else {
								...
								<tr>
									...
									>
										<button
											...
											type="button"
											on:click={goto(`/panel/action/update/${action._id.toString()}`)}
										>
											Düzenle
										</button>
										<button
											on:click={() => deleteActionApprove(action._id.toString())}
											class="bg-white text-blue-600 hover:bg-[#6e6e85] hover:text-white border border-blue-600 rounded font-bold  text-xs px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none "
											type="button"
										>
											Sil
