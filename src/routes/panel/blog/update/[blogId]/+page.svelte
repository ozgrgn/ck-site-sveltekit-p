<script>
	...
	const updateBlog = async () => {
		let editedBlog = {};
		values.map((v) => {
			editedBlog[v.key] = blog[v.key].value;
		});

		let response = await RestService.updateBlog(blog._id, editedBlog);
		if (response['status']) {
	...
	const getBlog = async () => {
		let response = await RestService.getBlog(data.blogId);

		if (response['status']) {
			values.map((v) => {
				if (v.customValue) {
					response['blog'][v.key] = {
						value: response['blog'][v.key][v.customValue]
					};
				} else {
					response['blog'][v.key] = { value: response['blog'][v.key] };
				}
			});
			blog = {
				...response['blog']
	...
	<div class="w-full mb-12 px-2 lg:px-4 ">
		<div class="flex justify-between">
			...
			<button
				class="bg-white text-blue-600 hover:text-red-700 mb-2 border rounded font-bold text-xs px-4 py-2 rounded shadow hover:shadow-lg outline-none focus:outline-none mr-1 "
				type="button"
				on:click={() => deleteBlogApprove(data.blogId)}
			>
				<i class="fa fa-trash" />
				Sil
			</button>
		...
		>
			<div class="rounded-t mb-0 px-4 py-3 border-0">
				<div class="text-center flex justify-between">
					<h3 class="font-semibold text-lg text-blueGray-700">Blog güncelle</h3>
				</div>
			</div>
