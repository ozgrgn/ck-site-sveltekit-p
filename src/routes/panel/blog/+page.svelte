<script>
	...
	const deleteBlogApprove = (blogId) => {
		modal.set(
			bind(Alert, {
	...
	const getBlogs = async (search) => {
		...
		if (getBlogsResponse['status']) {
			blogs = getBlogsResponse['blogs'];
			totalDataCount = getBlogsResponse['count'];
		} else {
	...
	const deleteBlog = async (blogId) => {
		let response = await RestService.deleteBlog(blogId);
		if (response['status']) {
			...
			<div class="rounded-t mb-0 px-4 py-3 border-0">
				<div class="flex flex-wrap items-center">
					<div class="relative w-full px-4 max-w-full flex-grow flex-1">
						<h3 class="font-semibold text-lg text-blueGray-700">Blog</h3>
					</div>
				</div>
			</div>
							...
							{#each blogs as blog}
								<tr>
									...
									<td
										class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-center"
									>
										{blog?.blogCat?.name ? blog?.blogCat?.name : '-'}
									</td>
									<td
										class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-center"
									>
										{blog?.treatment?.name ? blog.treatment.name : '-'}
									</td>
									<td
										class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-center"
									>
										{blog.homePage ? 'EVET' : 'HAYIR'}
									</td>
									<td
										class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-center"
									>
										{blog.blogHeader ? blog.blogHeader : '-'}
									</td>
									<td
										class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-center"
									>
										{blog.order ? blog.order : '-'}
									</td>
									...
									>
										<button
											...
											type="button"
											on:click={goto(`/panel/blog/update/${blog._id.toString()}`)}
										>
											Düzenle
										</button>
										<button
											on:click={() => deleteBlogApprove(blog._id.toString())}
											class="bg-white text-blue-600 hover:bg-[#6e6e85] hover:text-white border border-blue-600 rounded font-bold  text-xs px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none "
											type="button"
										>
											Sil
