<script>
	...
	const getBlogCats = async () => {
		let getBlogCatsResponse = await RestService.getBlogCats(lang, treatmentGroup, limit, skip);
		if (getBlogCatsResponse['status']) {
			blogCats = getBlogCatsResponse['blogCats'];
			totalDataCount = getBlogCatsResponse['count'];
		} else {
			...
			<div class="rounded-t mb-0 px-4 py-3 border-0">
				<div class="flex flex-wrap items-center">
					<div class="relative w-full px-4 max-w-full flex-grow flex-1">
						<h3 class="font-semibold text-lg text-blueGray-700">Blog Kategorisi</h3>
					</div>
				</div>
			</div>
							...
							{#each blogCats as blogCat}
								<tr>
									...
									<td
										class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-center"
									>
										{blogCat?.treatmentGroup?.name ? blogCat?.treatmentGroup?.name : '-'}
									</td>
									<td
										class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-center"
									>
										{blogCat.name ? blogCat.name : '-'}
									</td>
									...
									>
										<button
											...
											type="button"
											on:click={goto(`/panel/blogCat/update/${blogCat._id.toString()}`)}
										>
											Düzenle
										</button>
										<button
											on:click={() => deleteBlogCatApprove(blogCat._id.toString())}
											class="bg-white text-blue-600 hover:bg-[#6e6e85] hover:text-white border border-blue-600 rounded font-bold  text-xs px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none "
											type="button"
										>
											Sil
