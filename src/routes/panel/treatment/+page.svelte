<script>
	...
	const getTreatments = async () => {
		let getTreatmentsResponse = await RestService.getTreatments(lang, treatmentGroup, limit, skip);
		if (getTreatmentsResponse['status']) {
			treatments = getTreatmentsResponse['treatments'];
			totalDataCount = getTreatmentsResponse['count'];
		} else {
			ToastService.error($TranslateApiMessage(getTreatmentsResponse.message));
		}
	};
	onMount(() => {
		getTreatments();
							...
							{#each treatments as treatment}
								<tr>
									...
									<td
										class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-center"
									>
										{treatment.treatmentGroup.name ? treatment.treatmentGroup.name : '-'}
									</td>
									<td
										class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-center"
									>
									...
									>
										<button
											...
											type="button"
											on:click={goto(`/panel/treatment/update/${treatment._id.toString()}`)}
										>
											Düzenle
										</button>
										<button
											on:click={() => deleteTreatmentApprove(treatment._id.toString())}
											class="bg-white text-blue-600 hover:bg-[#6e6e85] hover:text-white border border-blue-600 rounded font-bold  text-xs px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none "
											type="button"
										>
											Sil
