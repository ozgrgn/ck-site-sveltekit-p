<script>
	...
	const getGenerals = async () => {
		let getGeneralsResponse = await RestService.getGenerals(undefined, limit, skip);
		if (getGeneralsResponse['status']) {
			generals = getGeneralsResponse['generals'];
			totalDataCount = getGeneralsResponse['count'];
		} else {
			...
			<div class="rounded-t mb-0 px-4 py-3 border-0">
				<div class="flex flex-wrap items-center">
					<div class="relative w-full px-4 max-w-full flex-grow flex-1">
						<h3 class="font-semibold text-lg text-blueGray-700">Genel Bilgiler</h3>
					</div>
				</div>
			</div>
								...
								<tr>
									...
									>
										<button
											...
											type="button"
											on:click={goto(`/panel/general/update/${general._id.toString()}`)}
										>
											Düzenle
										</button>
										<button
