<script>
	...
	const getTreatmentGroups = async () => {
		let getTreatmentGroupsResponse = await RestService.getTreatmentGroups(lang, limit, skip);
		if (getTreatmentGroupsResponse['status']) {
			treatmentGroups = getTreatmentGroupsResponse['treatmentGroups'];
			totalDataCount = getTreatmentGroupsResponse['count'];
		} else {
...
<div class="flex flex-wrap mt-4 h-screen relative">
	<div class="w-full mb-12 px-2 lg:px-4 ">
		<button
			...
			type="button"
			on:click={() => {
				goto('/panel/treatmentGroup/create');
			}}
		>
			<i class="fa fa-plus" />
			Ekle
			...
			<div class="block w-full overflow-x-auto">
				{#if treatmentGroups}
					<table class="items-center w-full bg-transparent border-collapse">
						...
						<tbody>
							{#each treatmentGroups as treatmentGroup}
								<tr>
									<td
									...
									<td
										class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-center"
									>
										{treatmentGroup.name ? treatmentGroup.name : '-'}
									</td>

									<td
										class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-center"
									>
										{treatmentGroup.order}
									</td>
									...
									>
										<button
											...
											type="button"
											on:click={goto(
												`/panel/treatmentGroup/update/${treatmentGroup._id.toString()}`
											)}
										>
											Düzenle
										</button>
										<button
											on:click={() => deleteTreatmentGroupApprove(treatmentGroup._id.toString())}
											class="bg-white text-blue-600 hover:bg-[#6e6e85] hover:text-white border border-blue-600 rounded font-bold  text-xs px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none "
											type="button"
										>
											Sil
